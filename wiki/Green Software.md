---
tags: ['#ref', '#wip']
---

- https://stratoflow.com/efficient-and-environment-friendly-programming-languages/
- https://learn.greensoftware.foundation/introduction
- https://patterns.greensoftware.foundation/
- [[Energy-Efficient Software Architecture for Developers]]
- rust + https://www.okami101.io/vuetify-admin/guide/crud/list.html#data-iterator
- https://github.com/pocketbase/pocketbase/tree/master
- casbin-rs + https://docs.spring.io/spring-security/site/docs/4.2.x/reference/html/appendix-schema.html
- https://gist.github.com/NoraCodes/e6d40782b05dc8ac40faf3a0405debd3
